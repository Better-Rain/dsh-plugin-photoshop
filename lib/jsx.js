...
/** Whether a layer carries a user layer mask. */
function dshLayerHasMask(layer) {
  try {
    var ref = new ActionReference();
    ref.putIdentifier(charIDToTypeID('Lyr '), layer.id);
    return executeActionGet(ref).hasKey(stringIDToTypeID('userMaskEnabled'));
  } catch (error) {
    return false;
  }
}
...
function dshAddMaskFromSelection() {
...
/**
 * Make a layer's mask the active channel, so a filter applies to the mask
 * rather than to the pixels. This is the one headless edge-refinement tool this
 * Photoshop offers: Layer > Matting, and Select and Mask with it, are not
 * scriptable here.
 */
function dshSelectMaskChannel() {
  var ref = new ActionReference();
...
/** Hand the selection back to the composite image, so later edits touch pixels. */
function dshSelectCompositeChannel() {
  try {
    var ref = new ActionReference();
    ref.putEnumerated(charIDToTypeID('Chnl'), charIDToTypeID('Chnl'), charIDToTypeID('Trgt'));
    var desc = new ActionDescriptor();
    desc.putReference(charIDToTypeID('null'), ref);
    desc.putBoolean(charIDToTypeID('MkVs'), false);
    executeAction(charIDToTypeID('slct'), desc, DialogModes.NO);
...
function dshCutoutOne(item, index) {
  ...
  try {
    ...
    if (DSH.mode === 'remove-background') {
      ...
      // Refine the selection before it becomes alpha. Contracting drops the rim
      // of background the subject was sitting on; feathering softens the
      // outline. Photoshop's Matting menu — the usual tool for that rim — is not
      // scriptable on this build, so contracting the selection stands in for it.
      if (DSH.contractPx > 0) {
        try { doc.selection.contract(UnitValue(DSH.contractPx, 'px')); }
        catch (contractError) { record.contractWarning = String(contractError.message); }
      }
      if (DSH.featherPx > 0) {
        try { doc.selection.feather(UnitValue(DSH.featherPx, 'px')); }
        catch (featherError) { record.featherWarning = String(featherError.message); }
      }
      dshUnlockBackground(doc);
      // A mask rather than a cleared selection: it keeps the selection's own
      // anti-aliasing, and it can be refined afterwards.
      dshAddMaskFromSelection();
      try { doc.selection.deselect(); } catch (deselectError) { }
      record.method = 'select-subject';
    }

    if (DSH.maskBlurPx > 0) {
      if (!dshLayerHasMask(doc.activeLayer)) {
        record.maskBlurWarning = 'the layer has no mask to blur';
      } else {
        dshSelectMaskChannel();
        doc.activeLayer.applyGaussianBlur(DSH.maskBlurPx);
        dshSelectCompositeChannel();
        record.maskBlurPx = DSH.maskBlurPx;
      }
    }
