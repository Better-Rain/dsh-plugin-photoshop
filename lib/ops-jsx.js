...
/** Notes an operation wants the caller to see, e.g. the files it wrote. */
var DSH_NOTES = [];

function dshNote(text) {
  if (DSH_NOTES.length < 50) DSH_NOTES.push(String(text));
}

/**
 * Turn a layer name into something a filesystem will accept. Written as a loop
 * rather than a regular expression because the character class would need three
 * levels of backslash escaping across the JavaScript and ExtendScript layers,
 * and a silently wrong class is not worth the brevity.
 */
function dshSafeName(text) {
...
var HANDLERS = {
   ...
   * Soften a layer mask.
   *
   * Photoshop's own edge tools — Select and Mask, the Matting menu — cannot run
   * headless on this build, but a mask is an ordinary grey channel, so blurring
   * it gives a controllable falloff on the cut edge. The mask has to be the
   * active channel for the filter to land on it rather than on the pixels.
   */
  refine_mask: function (op, doc) {
    var layer = dshTargetLayer(doc, op);
    var blur = dshNumber(op.blur_px, 'blur_px', 0);
    if (blur <= 0) throw new Error('refine_mask needs blur_px greater than 0');
    if (!dshLayerHasMask(layer)) throw new Error('"' + layer.name + '" has no layer mask to refine');
    dshSelectMaskChannel();
    try { layer.applyGaussianBlur(blur); }
    finally { dshSelectCompositeChannel(); }
    dshNote('softened the mask of "' + layer.name + '" by ' + blur + 'px');
  },

  clipping_mask: function (op, doc) {    var layer = dshTargetLayer(doc, op);
  ...
  // ── action ────────────────────────────────────────────────────────────────
  /**
   * Read the Actions panel. Photoshop exposes action sets by index, and the
   * actions of one set by a reference chained through that index — but the
   * chained form answers with a synthesised label rather than the action's real
   * name, so the usable names come from the unchained form, which enumerates
   * whichever set is currently targeted.
   *
   * Playing an action is deliberately not offered. A shipped action can open a
   * dialog, and a COM call blocked behind a modal cannot be cancelled from this
   * side: the timeout stops the caller while Photoshop keeps waiting. That is a
   * footgun, not a feature.
   */
  list_actions: function (op) {
    var sets = [];
    for (var s = 1; s <= 200; s++) {
      try {
        var setRef = new ActionReference();
        setRef.putIndex(stringIDToTypeID('actionSet'), s);
        var setDesc = executeActionGet(setRef);
        var setKey = stringIDToTypeID('name');
        sets.push(setDesc.hasKey(setKey) ? String(setDesc.getString(setKey)) : '(unnamed set)');
      } catch (error) { break; }
    }
    var actions = [];
    for (var a = 1; a <= 1000; a++) {
      try {
        ...
        actionRef.putIndex(stringIDToTypeID('action'), a);
        var actionDesc = executeActionGet(actionRef);
        var actionKey = stringIDToTypeID('name');
        actions.push(actionDesc.hasKey(actionKey) ? String(actionDesc.getString(actionKey)) : '(unnamed action)');
      } catch (error) { break; }
    }
    dshNote(sets.length === 0
      ? 'no action sets are available in the Actions panel'
      : 'action sets (' + sets.length + '): ' + sets.join(', '));
    dshNote(actions.length === 0
      ? 'the currently targeted action set is empty'
      : 'actions in the currently targeted set (' + actions.length + '): ' + actions.join(', '));
  },

  // ── export ────────────────────────────────────────────────────────────────
  /**
   * Write every layer of the document to its own file.
   *
   * Each layer is revealed on its own — everything else is hidden — and saved,
   * so the outputs keep the full canvas and line up with each other, which is
   * what Photoshop's own "Export Layers to Files" does and what makes the
   * results usable as a set. Every layer's original visibility is restored
   * afterwards, including when something fails.
   */
  export_layers: function (op, doc) {
    var directory = new File(String(op.output_dir));
    if (!directory.exists) {
      // ExtendScript's Folder does not reliably offer create() here, so the
      // caller pre-creates the directory; this is the backstop with a clear
      // message rather than a silent failure to write anything.
      try { directory.create(); } catch (createError) { }
    }
    if (!directory.exists) {
      throw new Error('the output directory does not exist: ' + directory.fsName + ' — create it before calling export_layers');
    }
    var format = op.format === undefined ? 'png' : String(op.format);
    var suffix = op.suffix === undefined ? '' : String(op.suffix);
    var visibleOnly = dshBool(op.visible_only, false);
    var recurse = dshBool(op.recursive, true);

    var targets = [];
    function dshCollect(container, ancestors) {
      for (var i = 0; i < container.length; i++) {
        var layer = container[i];
        if (String(layer.typename) === 'LayerSet') {
          if (recurse) dshCollect(layer.layers, ancestors.concat([layer]));
          continue;
        }
        if (visibleOnly && layer.visible !== true) continue;
        var chain = ancestors.concat([layer]);
        var parts = [];
        for (var c = 0; c < chain.length; c++) parts.push(String(chain[c].name));
        targets.push({ chain: chain, name: parts.join('-') });
      }
    }
    dshCollect(doc.layers, []);
    if (targets.length === 0) throw new Error('this document has no layers to export');

    var visibility = [];
    function dshSnapshot(container) {
      for (var i = 0; i < container.length; i++) {
        var layer = container[i];
        visibility.push({ layer: layer, visible: layer.visible === true });
        if (String(layer.typename) === 'LayerSet') dshSnapshot(layer.layers);
      }
    }
    dshSnapshot(doc.layers);

    var written = [];
    var used = {};
    var extension = format === 'jpeg' || format === 'jpg' ? 'jpg' : format === 'tiff' || format === 'tif' ? 'tif' : format;
    var baseDirectory = directory.fsName.replace(/\\\\/g, '/');
    try {
      for (var t = 0; t < targets.length; t++) {
        for (var v = 0; v < visibility.length; v++) visibility[v].layer.visible = false;
        for (var k = 0; k < targets[t].chain.length; k++) targets[t].chain[k].visible = true;

        var base = dshSafeName(targets[t].name) + suffix;
        var unique = base;
        var counter = 2;
        while (used[unique.toLowerCase()] === true) {
          unique = base + '-' + counter;
          counter++;
        }
        used[unique.toLowerCase()] = true;

        var path = baseDirectory + '/' + unique + '.' + extension;
        dshSaveAs(doc, path, format, 10, true);
        written.push(unique + '.' + extension);
      }
    } finally {
      for (var r = 0; r < visibility.length; r++) {
        try { visibility[r].layer.visible = visibility[r].visible; } catch (restoreError) { }
      }
    }
    dshNote('exported ' + written.length + ' layer(s) to ' + baseDirectory);
    dshNote(written.length <= 12 ? written.join(', ') : written.slice(0, 12).join(', ') + ', …and ' + (written.length - 12) + ' more');
  },
...
HANDLERS.list_actions.needsDocument = false;
...
/**
 * Resolve a plan's references without applying anything.
 *
 * Before a plan touches a real document it is worth knowing whether the layers
 * it names actually exist. This walks the plan and resolves every `document`,
 * `target` and `into` against the live state, reporting what each resolves to or
 * why it does not — which is the failure that actually happens in practice, a
 * layer name that was guessed rather than read from `photoshop_inspect`.
 *
 * It resolves references; it does not validate every field, because that lives in
 * the handlers and running them is the thing a dry run exists to avoid.
 * @param params - `resultPath` and `ops`.
 * @returns ExtendScript source.
 */
export function dryRunScript(params) {
  return `${prelude(params)}
${HANDLERS}
var out = { ok: true, tool: 'photoshop_dry_run', steps: [] };
try {
  var ops = DSH.ops;
  if (!(ops instanceof Array) || ops.length === 0) throw new Error('no operations were given');
  var opensDocument = false;
  for (var i = 0; i < ops.length; i++) {
    var op = ops[i];
    var step = { index: i, name: String(op === null || op === undefined ? '' : op.op), resolved: [], ok: false };
    try {
      var handler = HANDLERS[step.name];
      if (handler === undefined) throw new Error('unknown operation "' + step.name + '"');
      if (handler.needsDocument === false) {
        if (step.name === 'open' || step.name === 'new_document') opensDocument = true;
        step.note = step.name === 'open' || step.name === 'new_document'
          ? 'opens a document — later operations can only be checked once it exists'
          : 'does not need an open document';
      } else if (app.documents.length === 0 && opensDocument) {
        // The plan builds its own document, so its later references cannot be
        // checked against the current state. Saying they failed would be wrong.
        step.checkable = false;
        step.note = 'not checkable here — nothing is open yet and this plan opens a document';
      } else {
        var doc = dshDocumentFor(op);
        step.document = String(doc.name);
        if (op.target !== undefined && op.target !== null && String(op.target) !== '') {
          var target = null;
          try { target = dshResolveLayer(doc, op.target); }
          catch (targetError) { throw new Error('target "' + String(op.target) + '": ' + String(targetError.message)); }
          step.resolved.push('target "' + String(op.target) + '" -> "' + String(target.name) + '"');
        } else {
          step.resolved.push('target = the active layer, "' + String(doc.activeLayer.name) + '"');
        }
        if (op.into !== undefined && op.into !== null && String(op.into) !== '') {
          var into = null;
          try { into = dshResolveLayer(doc, op.into); }
          catch (intoError) { throw new Error('into "' + String(op.into) + '": ' + String(intoError.message)); }
          step.resolved.push('into "' + String(op.into) + '" -> "' + String(into.name) + '"');
        }
      }
      step.ok = true;
    } catch (error) {
      step.error = String(error.message ? error.message : error);
    }
    out.steps.push(step);
  }
} catch (error) {
