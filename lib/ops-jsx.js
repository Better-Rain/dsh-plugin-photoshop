...
var HANDLERS = {
  ...
  save_as: function (op, doc) {
    ...
    var format = String(op.format === undefined ? 'png' : op.format).toLowerCase();
    var target = new File(path);
    if (target.exists) {
      if (op.overwrite !== true) throw new Error('output exists: ' + path + ' (pass overwrite: true to replace it)');
      target.remove();
    }
    var options = null;
    if (format === 'png') { options = new PNGSaveOptions(); options.compression = 6; }
    else if (format === 'jpeg' || format === 'jpg') { options = new JPEGSaveOptions(); options.quality = dshNumber(op.quality, 'quality', 10); }
    else if (format === 'psd') { options = new PhotoshopSaveOptions(); }
    else if (format === 'tiff' || format === 'tif') { options = new TiffSaveOptions(); }
    else throw new Error('format must be png, jpeg, psd or tiff — got "' + format + '"');
    doc.saveAs(target, options, true, Extension.LOWERCASE);
    if (!target.exists) throw new Error('Photoshop reported success but wrote no file at ' + path);
  },
...
function dshRunOps(ops) {
  ...
  for (var i = 0; i < ops.length; i++) {
    ...
    handler(op, handler.needsDocument === false ? null : dshDocumentFor(op));
    DSH_CURRENT_OP = previous;
    DSH_RESULT.applied.push(name);
  }
