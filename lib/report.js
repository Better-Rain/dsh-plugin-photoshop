...
export function formatBatchReport(result, plan, summary = {}) {
  ...
  if (opaque.length > 0) {
    lines.push(
      `note: ${opaque.length} output(s) carry no alpha channel — expected when the plan ends by flattening onto a background, and otherwise a sign that the plan produced no transparency.`,
    )
  }
...
export function formatDryRunReport(result, plan) {
  ...
  for (const step of steps) {
    ...
    if (step.checkable === false) {
      lines.push(`  ${label} skip  ${step.note ?? 'not checkable here'}`)
  ...
  const failed = steps.filter((step) => step.ok !== true && step.checkable !== false)
  const unchecked = steps.filter((step) => step.checkable === false)
  lines.push('')
  if (failed.length > 0) {
    lines.push(`${failed.length} operation(s) name something that does not exist. Fix those before running the plan for real.`)
  } else if (unchecked.length > 0) {
    lines.push(
      `Every operation that could be checked resolves, and nothing was changed. ${unchecked.length} operation(s) work on a document this plan creates, so only running it can check those.`,
    )
  } else {
    lines.push('Every operation resolves against the current state, and nothing was changed. Remove dry_run to actually run it.')
  }
  return lines.join('\n')
}
