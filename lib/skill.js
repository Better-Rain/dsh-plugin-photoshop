...
**Putting a cutout on a plain white background**

Trainers and asset pipelines usually want an opaque white background rather than
transparency, which is one operation, not a new cutout:

\`\`\`
photoshop_batch {
  paths: ["D:/cutouts"],
  ops: [{ op: "flatten" }],
  output_dir: "D:/cutouts-white",
  output_format: "png"
}
\`\`\`

\`flatten\` fills every transparent pixel with pure white and merges to one layer —
verified: all four corners of the result are exactly 255,255,255 and the file
carries no alpha channel at all. The batch's "no alpha channel" note afterwards is
expected here, not a problem.

**Several subjects in one frame** are fine: Select Subject selects all of them, and
the background between them is removed. That is Photoshop's judgement, not a
mis-selection — the same frame cut with \`remove-background\` agrees.
...
export const SKILL = {
  ...
  // The registry validates this as a plain string; the object form that some
  // type declarations suggest is rejected at registration with a TypeError, which
  // shows up as the skill silently failing to load.
  source: 'dsh-plugin-photoshop',
  content: CONTENT,
}
