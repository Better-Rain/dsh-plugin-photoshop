...
import { applyScript, batchScript, dryRunScript } from './ops-jsx.js'
import { describeEdge, readPngStats } from './png.js'
import { REPORT_ITEM_LIMIT, formatApplyReport, formatBatchReport, formatDryRunReport, formatInspectReport } from './report.js'
...
/**
 * Capabilities this plugin has established as unavailable on the Photoshop it
 * was probed against — by running them, not by reading a reference. Reported by
 * `photoshop_status` so the agent does not promise the user something the tool
 * cannot deliver, and so a failure has an explanation before it happens.
 *
 * Every entry is reproducible with `test/probe-operations.mjs`; re-run it after
 * a Photoshop upgrade, because this list is a finding, not a constant.
 */
const VERIFIED_UNAVAILABLE = [
  'Select Focus Area — the command exists by name and refuses to run',
  'Camera Raw as a filter — not invocable, with or without a descriptor',
  'Neural Filters and Generative Fill — the commands are not available here',
  'The Layer > Matting menu: Defringe, Remove White Matte, Remove Black Matte, and colour decontamination',
  'Select and Mask / Refine Edge — a modal workspace, so use feather, contract and refine_mask instead',
  'Playing a recorded Action — it can raise a dialog that DialogModes.NO does not suppress and that blocks every later call',
  'Contact Sheet II — app.makeContactSheet always opens its dialog, whatever options it is given',
]
...
/**
 * Create the output directories a plan will write into.
 *
 * ExtendScript's Folder does not reliably offer a way to create a directory, so
 * the Node side does it before Photoshop is involved. Two shapes are covered: an
 * operation that declares an `output_dir` gets that directory, and `save_as`,
 * which names a single file, gets its parent. Missing the second one is not
 * obvious at the time — Photoshop reports it as "the Save command's parameters
 * are currently invalid", which says nothing about a missing directory.
 * @param plan - the operations about to run.
 */
function ensureOpDirectories(plan) {
  for (const op of plan) {
    if (op === null || typeof op !== 'object') continue
    if (typeof op.output_dir === 'string' && op.output_dir.trim() !== '') {
      try {
    ...
    if (op.op === 'save_as' && typeof op.path === 'string' && op.path.trim() !== '') {
      try {
        mkdirSync(dirname(resolve(op.path)), { recursive: true })
      } catch {
        /* as above */
      }
    }
    ...
    defineTool({
      ...
      async execute(_args, exec) {
        ...
        try {
          ...
          lines.push('')
          lines.push('Not available on this installation, established by running them:')
          for (const entry of VERIFIED_UNAVAILABLE) lines.push(`  - ${entry}`)
          lines.push('  (re-run test/probe-operations.mjs after a Photoshop upgrade; this is a finding, not a constant)')
    ...
    defineTool({
      ...
      parameters: {
        ...
        dry_run: {
          type: 'boolean',
          description:
            'Resolve every operation against the current state and report what it would touch, without applying anything. Use it when the plan names layers, to catch a wrong name before the document is modified. It checks references, not every field.',
        },
      ...
      async execute(args, exec) {
        ...
        try {
          if (args.dry_run === true) {
            const { result: dryResult, failure: dryFailure } = await roundTrip(
              session,
              dryRunScript({ resultPath: session.resultPath, ops: plan }),
              {
                timeoutMs: Number.isFinite(args.timeout_ms) && args.timeout_ms > 0 ? args.timeout_ms : defaultTimeoutMs(),
                signal: exec.signal,
              },
            )
            if (dryResult === undefined) return `Photoshop could not examine this plan.\n${dryFailure}`
            return formatDryRunReport(dryResult, plan)
          }
          const { result, failure } = await roundTrip(
            session,
            applyScript({
